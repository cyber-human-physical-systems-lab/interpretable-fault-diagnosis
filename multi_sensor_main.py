...
import copy
import numpy as np
from utils import shuffle_data_multi, load_data, init_formula, cal_robustness, gradient_decent
from itertools import combinations

dataset_path = os.path.join(os.getcwd(), 'datasets')
write_dir = os.path.join(os.getcwd(), 'outputs', 'multi')
os.makedirs(write_dir, exist_ok=True)

## multiple sensors
data_name = 'data_ADAPT'
...
shuffle_data_multi(cur_data_path, dataset_path, data_name)
time, signal, label, normalize_param = load_data(dataset_path, data_name, multi_flag)

## initialize formula
formula_list = []
for sensor_idx in range(signal['train'].shape[1]):
    init_f = init_formula(sensor_idx, time['train'][0], np.expand_dims(signal['train'][:, sensor_idx, :], axis=1))
    formula_list.extend(init_f)
...
while len(exist_formula_list) > 0:
    ...
    if top_accuracy >= 0.9 * cur_accuracy:
        ...
        if len(del_combo_list) > 0:
            ...
            print(f'exist formula: {exist_formula_list}. delete: {del_combo_list[cost_min_index]}')
            for ii in del_combo_list[cost_min_index]:
                exist_formula_list.remove(ii)
            print(f'after delete: {exist_formula_list}')
        else:
            break
    else:
        break
